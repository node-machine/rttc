 ...
 */
module.exports = function rebuildSanitized(val, allowNull, skipFunctions) {
  ...
  return _rebuild(val, allowNull, skipFunctions);
};
...
function _rebuild(val, allowNull, skipFunctions) {
  var stack = [];
  var keys = [];
  ...
  function _recursivelyRebuildAndSanitize (val, key) {
    ...
    else if (_.isFunction(val) && !skipFunctions){
      val = val.toString();
    }
