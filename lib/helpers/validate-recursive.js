...
module.exports = function _validateRecursive (expected, actual, errors, ensureSerializable){
  ...
  if (ensureSerializable && expected !== 'machine' && expected !== 'buffer' && expected !== 'stream' && !(_.isObject(actual) && actual instanceof Buffer)) {
    try {
  ...
  if (!expectedType.is(actual)){
    ...
    try {
      ...
      errors.push((function (){
        ...
        err.code = 'E_COERCION';
        console.log('E_COERCION ERROR:', err);
        return err;
      })());
    }
  }
