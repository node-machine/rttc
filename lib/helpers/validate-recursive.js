...
module.exports = function _validateRecursive (expected, actual, errors, ensureSerializable, meta, strict){
  ...
  if (!expectedType.is(actual)){
    ...
    var newErr = (function (){
      ...
      // This is considered a "minor" error if:
      //  • the actual value is primitive and we're not expecting a dictionary or an array
      //  • the actual value is a Date and we're expecting a string
      if (!isExpectingDictionary && !isExpectingArray && !_.isObject(actual)) {
        err.minor = true;
      }
      if (expected === 'string' && _.isDate(actual)) {
        err.minor = true;
      }
