...
module.exports = function _validateRecursive (expected, actual, errors, ensureSerializable, meta, strict){
  ...
  if (isExpectingArray) {
    ...
    // ...expecting ANY array ([])
    if (allowAnyArray) {
      return deepCloneAndSanitizeGenericArray(coercedValue);
    }
  ...
  if (isExpectingDictionary) {
    ...
    if (allowAnyDictionary){
      return deepCloneAndSanitizeGenericDictionary(coercedValue);
    }
    // ...expecting ANY dictionary ({})
 ...
 * it's already met the minimum requirement of being an object.  So we need to clone the
 * provided value for safety; and in the process ensure that it meets the basic minimum
 * quality requirements.
 */
function deepCloneAndSanitizeGenericDictionary(val){
  ...
  // Build a deep copy of the object
  // (removing keys with undefined values in the process)
  //
  // (note that the recursive validation will not penetrate deeper into this
  //  object, so we don't have to worry about doing unnecessary deep copies at
...
// TODO consider differentiating between `example: []` and `example: ['*']` --
//  • `[]` could be a generic array, which gets deep cloned
//  • then `['*']` could just be handled normally w/ recursive parsing (i.e. array is cloned, but its items are not)
 ...
 * it's already met the minimum requirement of being `_.isArray()`.  So we need to clone
 * the provided value for safety.
 */
function deepCloneAndSanitizeGenericArray(val){
  ...
  // Build a deep copy of the dictionary
  // (note that the recursive validation will not penetrate deeper into this
  //  object, so we don't have to worry about doing unnecessary deep copies at
  //  each successive level)
