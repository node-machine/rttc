...
module.exports = function _validateRecursive (expected, actual, errors, ensureSerializable){
  ...
  // - we're expecting a "machine", "buffer", or "stream"
  // - the actual value is a "buffer" (since buffers can be JSON-serialized-- however we don't allow it)
  // - the actual value is a "stream" (since streams can be JSON-serialized-- however we don't allow it)
  //
  // Notes:
  ...
  if (ensureSerializable && expected !== 'machine' && expected !== 'buffer' && expected !== 'stream' && !(_.isObject(actual) && actual instanceof Buffer) && !(_.isObject(actual) && actual instanceof Stream)) {
    try {
      actual = JSON.parse(JSON.stringify(actual));
      // console.log('Dehydrated actual value (JSON.parse(JSON.stringify()) ->',actual);
    }
    catch (e){
