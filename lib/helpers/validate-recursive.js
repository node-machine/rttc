...
module.exports = function _validateRecursive (expected, actual, errors, ensureSerializable){
  ...
  // - we're expecting a "machine", "buffer", or "stream" and the mystery meat looks right
  //
  // Notes:
  ...
  if (ensureSerializable &&
    ...
    // && !(_.isObject(actual) && actual instanceof Buffer) && !(_.isObject(actual) && actual instanceof Stream)) {
    ...
    if (
      ...
      // If we're not expecting a stream, but it looks like we got one, don't serialize it to JSON and back
      // (it will end up looking like a plain old object, which is confusing)
      // !(expected !== 'stream' && _.isObject(actual) && actual instanceof Stream)
    ) {
      try {
