  ...
  if (!expectedType.is(actual)){
    ...
    var newErr = (function (){
      ...
      if (_.isUndefined(actual)) {
        ...
        msg += 'Specified value (a ' + getDisplayType(actual) + ') ';
    ...
    try {
      ...
      errors.push((function (){
        var msg = '';
        if (_.isUndefined(actual)) {
          msg += 'Could not coerce the specified value (`undefined`) to the expected '+
          (_.isObject(expected)?getDisplayType(expected)+' ':'')+'type '+(_.isObject(expected)?'schema ':'')+': ' + compile(expected) + '';
        }
        else {
          msg += 'Could not coerce the specified value (a '+getDisplayType(actual)+') to the expected '+
          (_.isObject(expected)?getDisplayType(expected)+' ':'')+'type '+(_.isObject(expected)?'schema ':'')+': ' + compile(expected) + '';
        }
        if (meta && meta.keyName) {
          msg = 'For key `'+meta.keyName+'`: ' + msg;
