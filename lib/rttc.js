 ...
 */
function parseObject(input, value, options) {
  _.each(_.keys(input), function(key) {
    ...
    if(types.obj.is(_input)) {
      parseObject(_input, _value, options);
      return;
    }
...
function validateInputSchema(def, val, options) {
  ...
  if(!_.isPlainObject(def)) {
    val = coercePrimitive(def, val, { coerce: coerce, baseType: baseType });
    var valid = validatePrimitive(def, val);
    if(!valid) {
      var err = new Error();
      err.code = 'E_INVALID_TYPE';
  ...
  _.each(_.keys(def), function(inputName) {
    ...
    if(_.isArray(input.type)) {
      ...
      _.each(value, function(item) {
        ...
        if(types.obj.is(input.type[0])) {
          try {
            item = parseObject(input.type[0], item, { coerce: coerce, baseType: baseType });
          }
          catch (err) {
            err.code = 'E_INVALID_TYPE';
            err.message = util.format(
        ...
        // Handle an array of primitive values
          ...
          try {
            item = coercePrimitive(input.type[0], item, { coerce: coerce, baseType: baseType });
            var valid = validatePrimitive(input.type[0], item);
            if(!valid) {
    ...
    if(types.obj.is(input.type)) {
      ...
      try {
        value = parseObject(input.type, value, { coerce: coerce, baseType: baseType });
      }
      catch (e) {
        errors.push('Invalid type for input: ' + inputName);
        return;
    ...
    try {
      ...
      value = coercePrimitive(input.type, value, { coerce: coerce, baseType: baseType });
      var valid = validatePrimitive(input.type, value);
      if(!valid) {
        errors.push('Invalid type for input: ' + inputName);
        return;
      }
