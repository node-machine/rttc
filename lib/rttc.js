...
function validateAndOrCoerceObject(input, value, options) {
  ...
  _.each(_.keys(input), function(key) {
    ...
    // If the input is an array, do a proper recursive call back to validateInputs()
    if (types.arr.is(_input)) {
      return ({
        key: {
          type: _input
        }
    ...
    _value = coercePrimitive(_input, _value, { coerce: options.coerce || false, baseType: options.baseType || false });
